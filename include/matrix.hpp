...
namespace matrices
{
    ...
    private :
        Buffer_<Buffer_<double>> contSqMatrix_;
        size_t matrixSize_; 
        double det_ = std::nan("");
    ...
    public :
        ...
        SquareMatrix(const size_t size, T val = T{}) : contSqMatrix_(size, Buffer_<double>{size, static_cast<double>(val)}), 
                                                       matrixSize_(size) {};
        ...
        Buffer_<double>& operator[](const int indx) {return contSqMatrix_[indx];};
        const Buffer_<double>& operator[](const int indx) const {return contSqMatrix_[indx];};

        ~SquareMatrix() = default; 
        
        const size_t size() const noexcept {return matrixSize_;};
        
        const double determinant() requires std::is_arithmetic_v<T>;
        //  if determinant of the matrix has not been calculated 
        ...
        void swap_rows(Buffer_<double>& row1, Buffer_<double>& row2); 
...
//      SQUAREMATRIX
        ...
        : SquareMatrix<T>::SquareMatrix(size)
    {
        ...
        for(size_t rowIndx = 0; i != e && rowIndx < size; ++rowIndx)
            for(size_t colIndx = 0; colIndx < size; ++colIndx)
                contSqMatrix_[rowIndx][colIndx] = static_cast<double>(*i++);
    }
    ...
    template <typename T>
    const double 
    SquareMatrix<T>::determinant() requires std::is_arithmetic_v<T>
    {
    ...
    template <typename T>
    void 
    SquareMatrix<T>::swap_rows( SquareMatrix<T>::Buffer_<double>& row1, 
                                SquareMatrix<T>::Buffer_<double>& row2 )
    {
    ...
    template <typename T>
    void
    SquareMatrix<T>::rows_elementary_operations( SquareMatrix<T>::Buffer_<double>& row1, 
                                                 SquareMatrix<T>::Buffer_<double>& row2,    
